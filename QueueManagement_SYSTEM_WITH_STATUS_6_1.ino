...
void printAllQueues() {
    ...
    for (const auto& entry : entries) {
        ...
        switch(entry.status) {
            ...
            case 2: statusDesc = "AttendedTo "; break;
            case 3: statusDesc = "Re-Register "; break;
            case 4: statusDesc = "Resend "; break;
...
void processCardUnified(String uid, bool isLocalScan = true, int senderIndex = -1) {
    ...
    if (sharedQueue.getByUID(uid, existing)) {
        if (existing.status == 1) {
            // Accept returning patient with status 1 and upgrade to status 2
            sharedQueue.removeByUID(uid);
            item.status = 2;
            sharedQueue.add(uid, timeStr, pid, item.status);
            Serial.println("✅ Returning patient (status 1→2) re-registered.");
            if (isLocalScan) blinkLED(GREEN_LED_PIN);
        } 
        else if (existing.status == 2) {
            // Accept patient with status 2 and upgrade to status 3
            sharedQueue.removeByUID(uid);
            item.status = 3;
            sharedQueue.add(uid, timeStr, pid, item.status);
            Serial.println("✅ Returning patient (status 2→3) re-registered.");
            if (isLocalScan) blinkLED(GREEN_LED_PIN);
        }
        else {
            // Reject all other existing patients
            Serial.printf("⛔ Patient %s rejected (status %d).\n", uid.c_str(), existing.status);
            if (isLocalScan) blinkLED(RED_LED_PIN);
            return;
        }
    } else {
        // Accept new patient with status 0
        item.status = 0;
        sharedQueue.add(uid, timeStr, pid, 0);
        Serial.println("✅ New patient added (status 0).");
        if (isLocalScan) blinkLED(GREEN_LED_PIN);
    }

    // Broadcast/Send the update with sender exclusion
    if (isMaster) { //item.status = item.status-1;
...
void handleAddToQueue(const SourceInfo& source, const QueueItem& item, const String& uidStr) {
  // if (isMaster && l==1) {
  //     l=0;
  //       QueueItem update = item;
  //       update.addToQueue = (item.status != 4);
  //       update.removeFromQueue = (item.status == 4);
  //        update.status=update.status-1;
  //       for (int i = 0; i < numArrivalNodes; i++) {
  //           if (i != myIndex && i != source.srcIndex) {
  //               esp_now_send(arrivalMACs[i], (uint8_t*)&update, sizeof(update));
  //           }
  //       }
  //       Serial.println("📢 Broadcasted queue update");
  //   }
    ...
    // Process the add event based on status
    Serial.printf("\n🔄 Processing add request for %s (status %d)\n", uidStr.c_str(), item.status);

    // Remove existing entry if present
    if (sharedQueue.exists(uidStr)) {
        sharedQueue.removeByUID(uidStr);
        Serial.println("♻️ Removed existing entry");
    }
    Serial.println("♻️current  Status is: ");
 Serial.println(item.status);
    // Handle each status case
    switch (item.status) {
        case 0: // New patient
            sharedQueue.add(item.uid, item.timestamp, item.number, 0);
            Serial.printf("✅ Added NEW patient %s (status 0)\n", uidStr.c_str());
            break;

        case 1: // Called patient
            sharedQueue.add(item.uid, item.timestamp, item.number, 1);
            Serial.printf("🔄 Re-added CALLED patient %s (status 1)\n", uidStr.c_str());
            break;

        case 2: // Returning patient
            sharedQueue.add(item.uid, item.timestamp, item.number, 2);
            Serial.printf("🔄 Re-added RETURNING patient %s (status 2)\n", uidStr.c_str());
            break;

        case 3: // Ready patient
            sharedQueue.add(item.uid, item.timestamp, item.number, 3);
            Serial.printf("🔄 Re-added READY patient %s (status 3)\n", uidStr.c_str());
            break;

        case 4: // Completed patient
            Serial.printf("⏹️ COMPLETED patient %s removed (status 4)\n", uidStr.c_str());
            break;

        default:
            Serial.printf("⚠️ Unknown status %d for patient %s\n", item.status, uidStr.c_str());
            break;
    }
  Serial.println("♻️new  Status is: ");
 Serial.println(item.status);
    // Print updated queue for debugging
    printAllQueues();

    // If master, broadcast the update
...
//     processCardUnified(uidStr, false, source.srcIndex);
// }
void handleRemoveFromQueue(const SourceInfo& source, const QueueItem& item, const String& uidStr) {
    ...
    if (isMaster) {
        ...
        update.addToQueue = (entry.status != 4);
        update.removeFromQueue = (entry.status == 4);
...
void handleDoctorRequest(const SourceInfo& source) {
    ...
    static int status3Count = 0;  // Changed from status2 to status3
    static int status0Count = 0;
    const int MAX_STATUS_3 = 5;   // Changed from MAX_STATUS_2
    const int MAX_STATUS_0 = 3;

    // Get all patients and filter only status 0 and 3
    std::vector<QueueEntry> allPatients = sharedQueue.getAll();
    std::vector<QueueEntry> filteredPatients;
    
    for (const auto& patient : allPatients) {
        if (patient.status == 0 || patient.status == 3) {
            filteredPatients.push_back(patient);
        }
    }
    ...
    // Check availability of both statuses
    bool hasStatus0 = false;
    bool hasStatus3 = false;
    for (const auto& patient : filteredPatients) {
        if (patient.status == 0) hasStatus0 = true;
        if (patient.status == 3) hasStatus3 = true;
        if (hasStatus0 && hasStatus3) break;
    }

    // Selection logic
    if (hasStatus0 && hasStatus3) {
        if (status3Count < MAX_STATUS_3) {
            // Find oldest status 3 patient
            for (const auto& patient : filteredPatients) {
                if (patient.status == 3) {
            ...
            for (const auto& patient : filteredPatients) {
                if (patient.status == 0) {
                    ...
                    foundPatient = true;
                    status0Count++;
                    if (status0Count == 1) status3Count = 0; // Reset counter
                    break;
                }
            }
        }
        ...
        for (const auto& patient : filteredPatients) {
            if ((hasStatus3 && patient.status == 3) || 
                (hasStatus0 && patient.status == 0)) {
                patientToSend = patient;
                foundPatient = true;
                break;
            }
    ...
    if (status3Count >= MAX_STATUS_3 && status0Count >= MAX_STATUS_0) {
        status3Count = 0;
        status0Count = 0;
    }
    ...
    if (patientToSend.status == 0) {
        ...
        Serial.printf("✅ Called patient %s (0→1), requeued\n", patientToSend.uid);
         Serial.print("✅ Called patient %s (0→1)" );
          Serial.println( patientToSend.status);
    } 
    else if (patientToSend.status == 3) {
        patientToSend.status = 4;
        sharedQueue.add(patientToSend.uid, patientToSend.timestamp, 
                       patientToSend.number, patientToSend.status);
        Serial.printf("✅ Completed patient %s (3→4)\n", patientToSend.uid);
        Serial.print("✅ Completed patient %s (3→4)\  ");
        Serial.println(patientToSend.status);
    }
    ...
    if (isMaster) {
        ...
        update.addToQueue = (patientToSend.status != 4);
        update.removeFromQueue = (patientToSend.status == 4);
...
void notifyNoPatients(int doctorNodeID) {
    ...
    item.removeFromQueue = false;
                for (int i = 0; i < numDoctorNodes; ++i) {
                   item.node = i+1;
                    esp_now_send(doctorMACs[i], (uint8_t*)&item, sizeof(item));
                }
...
void handleDoctorRemoval(const SourceInfo& source, const QueueItem& item, const String& uidStr) {
    Serial.println("⚠️ In handleDoctorRemoval");
    ...
    Serial.printf("Current status for %s: %d\n", uidStr.c_str(), entry.status);

    // Handle status 4 (remove completely)
    if (entry.status == 4) {
        sharedQueue.removeByUID(uidStr);
        ...
        if (isMaster) {
            QueueItem updateMsg;
            strncpy(updateMsg.uid, uidStr.c_str(), sizeof(updateMsg.uid));
            updateMsg.number = entry.number;
            updateMsg.status = 4;
            updateMsg.addToQueue = false;
            updateMsg.removeFromQueue = true;
            
            // Broadcast removal to all nodes
            for (int i = 0; i < numArrivalNodes; i++) {
                if (i != myIndex && i != source.srcIndex) {
                    esp_now_send(arrivalMACs[i], (uint8_t*)&updateMsg, sizeof(updateMsg));
                }
            }
    ...
    // Handle status 2 (update to 3)
    if (entry.status == 1) {
        // Remove old entry
        sharedQueue.removeByUID(uidStr);
        
        // Create updated entry
        QueueEntry updatedEntry = entry;
        updatedEntry.status = 2;
        ...
        Serial.printf("🔄 Doctor %d updated patient %s status: 2 → 3\n",
                    source.doctorNodeID, uidStr.c_str());

        if (isMaster) {
            QueueItem updateMsg;
            strncpy(updateMsg.uid, uidStr.c_str(), sizeof(updateMsg.uid));
            updateMsg.number = updatedEntry.number;
            updateMsg.status = 3;
            updateMsg.addToQueue = true;
            updateMsg.removeFromQueue = false;
    ...
    Serial.printf("ℹ️ Patient %s has status %d - no action taken\n", 
                uidStr.c_str(), entry.status);
    printAllQueues();
}
void handleClearQueue(const SourceInfo& source) {
